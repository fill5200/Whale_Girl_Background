...
- **插件机制契约**：`contributes.tools` 与入口注册的工具**逐名一致**（缺失即启用失败回滚）；bundle 的 `id` 必须等于插件 id（`vlln/dsh-pet`）；`client.inject` 只是图元数据，fiber 实际依赖由 bundle 自导出决定。
