...
export function apply(ctx = {}) {
  ...
  bgBtn.textContent = '🖼️ 背景'
  bgBtn.style.cssText = BTN_STYLE
  menu.append(feedBtn, playBtn, roleBtn, bgBtn)

  // ---- 背景功能（本地 fork 增强）：DSH 全屏背景图 + 透明度调节 ----
  // 契约：透明度滑条 0-100 —— 100% = 默认背景（图片完全透明），0% = 完整图片背景。
  // 实现：全屏 fixed 背景层（z-index:-1，pointer-events:none）铺图；同时把主题底色
  // 变量（frame 的 --dsw-alias-bg-base、侧边栏的 --dsw-specific-sidebar-fill）覆盖为
  // transparent，让默认底色不遮挡图片；透明度 100% 时恢复变量（显示默认背景）。
  // 主题服务在 theme/change 时重写 body 内联变量 → MutationObserver 兜底重新覆盖。
  // 持久化：localStorage（与角色/位置同一模式），key whale-girl:background。
  const BG_STORAGE_KEY = 'whale-girl:background'
  const BG_VARS = ['--dsw-alias-bg-base', '--dsw-specific-sidebar-fill']
  // 背景层：全屏、内容之下、不拦事件。
  const bgLayer = document.createElement('div')
  bgLayer.style.cssText = 'position: fixed; inset: 0; z-index: -1; pointer-events: none; background-size: cover; background-position: center; background-repeat: no-repeat;'
  document.body.appendChild(bgLayer)
  // 面板：锚定宠物下方（菜单再下 40px），内联样式防宿主清理。
  ...
  bgPanel.style.cssText = [
    ...
    'width: 220px; padding: 8px; box-sizing: border-box;',
    `background: ${PANEL_THEME.bg}; border: 1px solid ${PANEL_THEME.border}; box-shadow: ${PANEL_THEME.shadow};`,
    `color: ${PANEL_THEME.text}; font-size: ${PANEL_THEME.font}; border-radius: ${PANEL_THEME.radius};`,
    'z-index: 6; display: none; pointer-events: auto; font-family: system-ui, sans-serif;',
  ].join(' ')
  const bgImportBtn = document.createElement('button')
  bgImportBtn.textContent = '📁 导入图片'
  bgImportBtn.style.cssText = BTN_STYLE
  const bgResetBtn = document.createElement('button')
  bgResetBtn.textContent = '↩️ 还原默认'
  bgResetBtn.style.cssText = BTN_STYLE
  const bgRow = document.createElement('div')
  bgRow.style.cssText = 'display: flex; gap: 6px; margin-bottom: 6px;'
  bgRow.append(bgImportBtn, bgResetBtn)
  const bgSliderLabel = document.createElement('span')
  bgSliderLabel.textContent = '透明度'
  bgSliderLabel.style.cssText = 'flex: none;'
  const bgSlider = document.createElement('input')
  bgSlider.type = 'range'
  bgSlider.min = '0'
  bgSlider.max = '100'
  bgSlider.step = '1'
  bgSlider.style.cssText = 'flex: 1; min-width: 0; accent-color: #5a86fe;'
  const bgSliderValue = document.createElement('span')
  bgSliderValue.style.cssText = 'flex: none; width: 34px; text-align: right; font-variant-numeric: tabular-nums;'
  const bgSliderRow = document.createElement('div')
  bgSliderRow.style.cssText = 'display: flex; align-items: center; gap: 6px;'
  bgSliderRow.append(bgSliderLabel, bgSlider, bgSliderValue)
  bgPanel.append(bgRow, bgSliderRow)
  ...
  // 状态：{ dataUrl, transparency } | null
  let bgState = null
  try {
    const saved = JSON.parse(localStorage.getItem(BG_STORAGE_KEY) ?? 'null')
    if (saved !== null && typeof saved === 'object' && typeof saved.dataUrl === 'string' && Number.isFinite(saved.transparency)) {
      bgState = { dataUrl: saved.dataUrl, transparency: Math.min(100, Math.max(0, saved.transparency)) }
    }
  } catch {}
  // 应用背景：图层透明度 + 主题变量覆盖/恢复。
  const applyBackground = () => {
    const hasImage = bgState !== null
    const v = bgState?.transparency ?? 100
    bgLayer.style.backgroundImage = hasImage ? `url("${bgState.dataUrl}")` : 'none'
    bgLayer.style.opacity = hasImage ? String((100 - v) / 100) : '0'
    const active = hasImage && v < 100
    for (const name of BG_VARS) {
      if (active) document.body.style.setProperty(name, 'transparent')
      else document.body.style.removeProperty(name)
    }
  }
  // 主题服务重写 token 兜底：body 内联样式变化后重新覆盖（值相同不重设防循环）。
  const bgObserver = new MutationObserver(() => {
    const active = bgState !== null && bgState.transparency < 100
    for (const name of BG_VARS) {
      const want = active ? 'transparent' : ''
      if (document.body.style.getPropertyValue(name) !== want) {
        if (active) document.body.style.setProperty(name, 'transparent')
        else document.body.style.removeProperty(name)
      }
    }
  ...
  const renderBgPanel = () => {
    const v = bgState?.transparency ?? 100
    bgSlider.value = String(v)
    bgSliderValue.textContent = `${v}%`
    bgResetBtn.disabled = bgState === null
  }
  const saveBg = () => {
    try {
      if (bgState === null) localStorage.removeItem(BG_STORAGE_KEY)
      else localStorage.setItem(BG_STORAGE_KEY, JSON.stringify(bgState))
    } catch {}
  }
  // 面板开关：点背景按钮 toggle；关闭菜单时同步关闭（toggleMenu 内联控制权威）。
  const toggleBgPanel = (open) => {
    const next = open ?? bgPanel.style.display === 'none'
    bgPanel.style.display = next ? 'block' : 'none'
    if (next) renderBgPanel()
    return next
  }
  bgBtn.addEventListener('click', () => toggleBgPanel())
  ...
  bgSlider.addEventListener('input', () => {
    ...
    bgState.transparency = Number(bgSlider.value)
    saveBg()
    applyBackground()
    renderBgPanel()
  })
  bgFileInput.addEventListener('change', async () => {
    ...
    try {
      ...
      bgState = { dataUrl, transparency: 0 } // 导入后默认完整图片（透明度 0%）
      saveBg()
      applyBackground()
      renderBgPanel()
    } catch {}
  })
  // canvas 压缩：最长边 ≤1600，jpeg 0.85（控制 localStorage 体积；背景图场景无需透明）。
  const compressToDataUrl = (file) => new Promise((resolve) => {
  ...
  const toggleMenu = (open) => {
    ...
    if (!next) toggleBgPanel(false) // 菜单关闭 → 背景面板同步收起（防残留浮层）
    if (next) {
      statusForcedHidden = true
      setStatusVisible(false)
  ...
  const dispose = () => {
    ...
    bgObserver.disconnect() // 背景功能清理：观察器、背景层、面板、文件输入、主题变量恢复
    bgLayer.remove()
    bgPanel.remove()
    bgFileInput.remove()
    for (const name of BG_VARS) document.body.style.removeProperty(name)
    host.remove()
    style.remove()
  }
  return dispose
