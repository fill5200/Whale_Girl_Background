...
启用后刷新 Web 页面，右下角出现宠物：点击弹出菜单（🍗 投喂 / 🎾 玩耍），拖拽可移动；状态条显示资历（等级/任务数）与最近共同回忆，每 3s 轮询刷新。任务完成时宠物庆祝（页面关闭期间完成的任务重开后也会庆祝）、失败时短暂失落（`error`→`disappointed` 瞬发）、新会话时挥手欢迎；任一会话运行/思考时宠物沉思陪伴（`think`），等待批准时抬眼期待（`wait`）。
