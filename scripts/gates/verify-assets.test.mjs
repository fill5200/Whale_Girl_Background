...
function writeSheets(root, dir = 'assets') {
  for (const name of STATE_NAMES) {
...
test('接受：全 15 状态 + sheet 全存在且字段合法', () => {
  const root = makeTree({ 'assets/manifest.json': fullStates() })
  writeSheets(root)
...
test('拒绝：manifest 引用不存在的 sheet（含状态名与路径）', () => {
  const root = makeTree({ 'assets/manifest.json': fullStates() }) // 不写 sheet 文件
  const { ok, errors } = check(root)
  assert.equal(ok, false)
  assert.match(errors.join('\n'), /states\.idle/)
  assert.match(errors.join('\n'), /idle\.png/)
...
test('拒绝：单姿势 256×256 配 frames:2（宽度 ≠ 2×高度，姿势会被劈开）', () => {
  ...
  writeFileSync(join(root, 'assets', 'eat.png'), fakePng(256, 256))
  const { ok, errors } = check(root)
  assert.equal(ok, false)
...
test('接受：角色索引（characters + default）且 sheet 在角色目录', () => {
  ...
  writeSheets(root, 'assets/characters/cat')
  const { ok, errors } = check(root)
  assert.equal(ok, true, errors.join('\n'))
})
...
test('拒绝：角色 id 非法（URL 注入面）', () => {
  ...
  const root = makeTree({ 'assets/manifest.json': bad })
  writeSheets(root, 'assets/characters/a/b')
  const { ok, errors } = check(root)
  assert.equal(ok, false)
  assert.match(errors.join('\n'), /角色 id 只允许/)
})
