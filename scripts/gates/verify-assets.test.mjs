...
import { STATE_NAMES } from '../../.dsh-plugin/client/logic.mjs'
...
function writeSheets(root, dir = '.dsh-plugin/assets') {
  for (const name of STATE_NAMES) {
...
test('接受：全 15 状态 + sheet 全存在且字段合法', () => {
  const root = makeTree({ '.dsh-plugin/assets/manifest.json': fullStates() })
  writeSheets(root, '.dsh-plugin/assets')
...
test('拒绝：缺必备状态（素材必须全量，不再 emoji 降级）', () => {
  ...
  const root = makeTree({ '.dsh-plugin/assets/manifest.json': states })
  writeSheets(root, '.dsh-plugin/assets')
  const { ok, errors } = check(root)
...
test('拒绝：manifest 引用不存在的 sheet（含状态名与路径）', () => {
  const root = makeTree({ '.dsh-plugin/assets/manifest.json': fullStates() }) // 不写 sheet 文件
  const { ok, errors } = check(root)
  assert.equal(ok, false)
  assert.match(errors.join('\n'), /states\.idle/)
  assert.match(errors.join('\n'), /idle\.png/)
...
test('拒绝：manifest 不是合法 JSON', () => {
  const root = makeTree({ '.dsh-plugin/assets/manifest.json': '{broken' })
  const { ok, errors } = check(root)
  assert.equal(ok, false)
  assert.match(errors.join('\n'), /无法解析/)
})
...
test('拒绝：frames 非法', () => {
  ...
  const root = makeTree({ '.dsh-plugin/assets/manifest.json': bad })
  writeSheets(root, '.dsh-plugin/assets')
  const { ok, errors } = check(root)
  assert.equal(ok, false)
  assert.match(errors.join('\n'), /frames 必须是正整数/)
...
test('接受：frames:1 + 白名单 motion 配方', () => {
  ...
  const root = makeTree({ '.dsh-plugin/assets/manifest.json': good })
  writeSheets(root, '.dsh-plugin/assets')
  const { ok, errors } = check(root)
  assert.equal(ok, true, errors.join('\n'))
})
...
test('拒绝：单姿势 256×256 配 frames:2（宽度 ≠ 2×高度，姿势会被劈开）', () => {
  ...
  writeFileSync(join(root, '.dsh-plugin', 'assets', 'eat.png'), fakePng(256, 256))
  const { ok, errors } = check(root)
  assert.equal(ok, false)
...
test('接受：角色索引（characters + default）且 sheet 在角色目录', () => {
  ...
  const root = makeTree({ '.dsh-plugin/assets/manifest.json': good })
  writeSheets(root, '.dsh-plugin/assets/characters/whale-girl')
  writeSheets(root, '.dsh-plugin/assets/characters/cat')
  const { ok, errors } = check(root)
  assert.equal(ok, true, errors.join('\n'))
})
...
test('拒绝：角色 sheet 文件缺失（期望在 characters/<id>/ 下）', () => {
  ...
  const root = makeTree({ '.dsh-plugin/assets/manifest.json': bad, 'assets/idle.png': 'x' }) // 平铺不算数
  const { ok, errors } = check(root)
  assert.equal(ok, false)
...
test('拒绝：角色 id 非法（URL 注入面）', () => {
  ...
  const root = makeTree({ '.dsh-plugin/assets/manifest.json': bad })
  writeSheets(root, '.dsh-plugin/assets/characters/a/b')
  const { ok, errors } = check(root)
  assert.equal(ok, false)
  assert.match(errors.join('\n'), /角色 id 只允许/)
})
...
test('接受：四种 playback 模式 + 帧数下限满足', () => {
  ...
  writeFileSync(join(root, '.dsh-plugin', 'assets', 'idle.png'), fakePng(768, 256))
  writeFileSync(join(root, '.dsh-plugin', 'assets', 'walk.png'), fakePng(768, 256))
  const { ok, errors } = check(root)
  assert.equal(ok, true, errors.join('\n'))
})
