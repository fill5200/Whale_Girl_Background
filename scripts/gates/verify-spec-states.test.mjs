...
const emoji = Object.keys(EMOJI)

/** 构造一份与当前 EMOJI 表完全一致的 spec 总表。 */
function goodSpec() {
  const rows = emoji.map((s) => `| \`${s}\` | 触发 | 1 | — | — | 画面 |`)
  return [
    '# Sprite 素材规格（生图契约）',
    '',
    '## 状态总表（权威，15 状态）',
...
test('接受：spec 总表与 EMOJI 表逐名一致、声明数正确', () => {
  const { ok, errors } = check(makeTree(goodSpec()))
  assert.equal(ok, true, errors.join('\n'))
})

test('拒绝：EMOJI 有但 spec 缺（新增状态漏同步 spec）', () => {
  const rows = emoji.filter((s) => s !== 'think').map((s) => `| \`${s}\` | 触发 | 1 | — | — | 画面 |`)
  const spec = [
    '## 状态总表（权威，14 状态）',
    '',
    '| 状态 | 帧数 |',
  ...
  assert.match(errors.join('\n'), /client EMOJI 有 think/)
})

test('拒绝：spec 有但 EMOJI 缺（删除/改名状态漏同步 logic.mjs）', () => {
  const rows = [...emoji, 'teleport'].map((s) => `| \`${s}\` | 触发 | 1 | — | — | 画面 |`)
  const spec = [
    '## 状态总表（权威，16 状态）',
    '',
    '| 状态 | 帧数 |',
